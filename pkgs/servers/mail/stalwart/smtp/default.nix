...
rustPlatform.buildRustPackage {
  ...
  src = fetchFromGitHub {
    ...
    repo = pname;
    rev = "v${version}";
    sha256 = "sha256-YfKsnkJdotK+m7jX/mI40asUv6QKNuzl8RtyFDTRZXQ=";
  ...
  buildInputs = lib.optionals stdenv.isDarwin [ SystemConfiguration Security ];

  passthru.updateScript = nix-update-script { };

  checkFlags = [
    # access to the file system, but not sure which file
    "--skip=tests::management::queue::manage_queue"
    "--skip=tests::management::report::manage_reports"
    "--skip=tests::queue::manager::queue_due"
    "--skip=tests::reporting::tls::report_tls"
  ];
